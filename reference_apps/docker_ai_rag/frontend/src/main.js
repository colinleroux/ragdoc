  ...
  <main class="mx-auto grid min-h-screen w-full max-w-7xl grid-cols-1 gap-5 p-4 lg:grid-cols-[1.7fr_1fr]">
    ...
    <aside class="panel min-h-[85vh] overflow-hidden">
      ...
      <div class="h-[calc(85vh-70px)] space-y-4 overflow-auto p-4">
        ...
        <div id="model-progress-wrap" class="hidden rounded-xl border border-slate-800 bg-slate-900 p-3">
          <div id="model-progress-text" class="mb-2 text-xs text-slate-300">Preparing model pull...</div>
          <div class="h-2 overflow-hidden rounded-full bg-slate-800">
            <div id="model-progress-bar" class="h-full w-0 bg-cyan-400 transition-all duration-200"></div>
          </div>
        </div>
...
async function sendMessage(message) {
  ...
  try {
    ...
    if (String(err.message || "").toLowerCase().includes("not found")) {
      appendMessage("bot", 'A model appears missing. Click "Setup Models" to pull required Ollama models, then retry.');
    }
...
async function ingestDocs() {
  ...
  try {
    ...
    if (String(err.message || "").toLowerCase().includes("not found")) {
      appendMessage("bot", 'A model appears missing. Click "Setup Models" to pull required Ollama models, then retry ingest.');
    }
...
async function setupModels() {
  ...
  modelProgressText.textContent = "Starting model pull...";

  const setProgress = (overallPercent, text) => {
    const safe = Math.max(0, Math.min(100, Number(overallPercent || 0)));
    modelProgressBar.style.width = `${safe}%`;
  ...
  const stream = new EventSource("/api/setup-models/stream");
  let finished = false;

  stream.addEventListener("start", () => {
    setProgress(1, "Connected. Waiting for first progress update...");
  });

  stream.addEventListener("progress", (event) => {
    const data = JSON.parse(event.data || "{}");
    const overall = data.overall_percent ?? 0;
    const modelLabel = data.model_type || "model";
    const modelPercent = data.model_percent ?? 0;
    const status = data.status || "pulling";
    setProgress(overall, `${modelLabel}: ${status} (${modelPercent}%) | overall ${overall}%`);
  });
  ...
  stream.addEventListener("failed", (event) => {
    const data = JSON.parse(event.data || "{}");
    appendMessage("bot", `Model setup error: ${data.detail || "Model setup failed"}`);
    statusEl.textContent = "Model setup failed";
    finished = true;
    stream.close();
    setupModelsBtn.disabled = false;
  ...
  stream.onerror = () => {
    if (!finished) {
      appendMessage("bot", "Model setup stream disconnected before completion.");
      statusEl.textContent = "Model setup interrupted";
      setupModelsBtn.disabled = false;
      stream.close();
    }
  };
